...
export function CompanyNav() {
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-white/10 bg-[#071124]">
      <Container size="2xl">
        <div className="flex items-center justify-between gap-4 py-2">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="relative md:hidden">
              <Button
                variant="outline"
                className="border-white/25 text-white hover:border-[#D4AF37] hover:text-[#D4AF37] bg-white/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#D4AF37] focus-visible:ring-offset-2 focus-visible:ring-offset-[#071124] min-w-[44px] min-h-[44px]"
                onClick={() => setOpen((v) => !v)}
                aria-haspopup="true"
                aria-expanded={open}
              ...
              {open && (
                <div className="absolute right-0 mt-2 w-52 rounded-lg border border-white/10 bg-[#0b1a33] shadow-lg shadow-black/30 p-2 space-y-1 text-sm z-50">
                  {linkKeys.map((item) => (
                    <Link
                      key={item.to}
                      to={item.to}
