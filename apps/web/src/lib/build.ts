...
export function isRootBuild(): boolean {
  ...
  if (typeof window !== 'undefined') {
    ...
    // If pathname starts with /mineiro, definitely the Mineiro build
    if (pathname.startsWith('/mineiro')) {
      return false;
    }
    // If pathname doesn't start with /mineiro and we're on a /company/* route,
